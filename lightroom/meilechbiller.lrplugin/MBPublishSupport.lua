--[[----------------------------------------------------------------------------
MBPublishSupport.lua
Publish-specific hooks: collection settings (client + gallery type), comments
and favorites sync, deletes, renames, "Open gallery" links.
------------------------------------------------------------------------------]]
...
publishServiceProvider.titleForPublishedSmartCollection = 'Smart Gallery'
publishServiceProvider.titleForPublishedSmartCollection_standalone = 'Smart Gallery'
...
function publishServiceProvider.viewForCollectionSettings( f, publishSettings, info )
	...
	LrTasks.startAsyncTask( function()
		local ok, clients = pcall( MBApi.listClients, publishSettings )
		if ok then
			settings.clientItems = clientItemsFrom( clients )
			settings.clientsStatus = string.format( '%d client(s) on the site', #clients )
		else
	...
	return f:group_box {
		...
		f:row {
			...
			f:static_text { title = bind 'clientsStatus', text_color = import 'LrColor'( 0.4, 0.4, 0.4 ), fill_horizontal = 1 },
		},
		f:row {
			f:static_text { title = 'New client name:', width = 110, alignment = 'right' },
			f:edit_field { value = bind 'newClientName', fill_horizontal = 1,
				enabled = LrBinding.keyEquals( 'clientId', '' ) },
		},
		f:row {
			f:static_text { title = 'New client email:', width = 110, alignment = 'right' },
			f:edit_field { value = bind 'newClientEmail', fill_horizontal = 1,
				enabled = LrBinding.keyEquals( 'clientId', '' ) },
		},
		f:row {
		...
		f:row {
			...
			f:static_text {
				title = linked
					and 'This collection is linked to a gallery on the site. Rename the collection to rename the gallery.'
...
--- Fetches notes + favorites for a batch of published photos in one call.
local function loadFeedback( publishSettings, arrayOfPhotoInfo )
	...
	local ok, data = pcall( MBApi.feedback, publishSettings, ids )
	if not ok then
...
local function isoToTime( iso )
	...
	-- os.time interprets the table as local time; the offset only affects display order.
...
function publishServiceProvider.getCommentsFromPublishedCollection( publishSettings, arrayOfPhotoInfo, commentCallback )
	...
	for _, photoInfo in ipairs( arrayOfPhotoInfo ) do
		...
		for _, c in ipairs( comments ) do
			local who = c.author_role == 'admin' and 'You' or ( c.author_name or 'Client' )
			list[ #list + 1 ] = {
				commentId = c.id,
				commentText = ( c.resolved and '✓ ' or '' ) .. ( c.body or '' ),
...
function publishServiceProvider.getRatingsFromPublishedCollection( publishSettings, arrayOfPhotoInfo, ratingCallback )
	...
	-- Tag favorites with a keyword so they can be filtered in the Library.
	if publishSettings.tagFavorites ~= false and #toTag > 0 then
		...
		pcall( function()
			catalog:withWriteAccessDo( 'Tag client favorites', function()
				...
				if keyword then
					for _, photo in ipairs( toTag ) do photo:addKeyword( keyword ) end
				end
			end, { timeout = 15 } )
		end )
	end
...
function publishServiceProvider.canAddCommentsToService( publishSettings )
	local ok = pcall( MBApi.ping, publishSettings )
	return ok
end

function publishServiceProvider.addCommentToPublishedPhoto( publishSettings, remotePhotoId, commentText )
	local ok, err = pcall( MBApi.addComment, publishSettings, remotePhotoId, commentText )
	if not ok then LrDialogs.message( 'Could not post your reply', tostring( err ), 'warning' ) end
	return ok
end
...
function publishServiceProvider.deletePhotosFromPublishedCollection( publishSettings, arrayOfPhotoIds, deletedCallback )
	for _, photoId in ipairs( arrayOfPhotoIds ) do
		local ok, err = pcall( MBApi.deletePhoto, publishSettings, photoId )
...
function publishServiceProvider.renamePublishedCollection( publishSettings, info )
	if info.remoteId then
		local ok, err = pcall( MBApi.updateGallery, publishSettings, info.remoteId, { title = info.name } )
		if not ok then LrDialogs.message( 'Could not rename the gallery', tostring( err ), 'warning' ) end
	end
end

function publishServiceProvider.deletePublishedCollection( publishSettings, info )
	-- Photos stay on the site; the gallery is archived so the client link stops working.
	if info.remoteId then
		pcall( MBApi.updateGallery, publishSettings, info.remoteId, { status = 'archived' } )
	end
end
...
MBPublishSupport = publishServiceProvider
